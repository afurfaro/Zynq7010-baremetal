...
><span style="color:green">:heavy_check_mark:</span>**```LR```** = **```LR_svc```** = Dirección de retorno (Instrucción posterior a SVC]. La razón es que la llamada se detecta en la fase de Decodificación del pipeline y por lo tanto solo se suma 4 al **```PC```** una sola vez. En este caso particular, esta es la razón por la que **```SVC```** retorna a la instrucción correcta. Veremos en futuros experimentos que éste no es el caso mas común, y que de hecho el programador debe decrementar en 4 u 8 bytes la dirección de retorno en **```LR```** antes de aplicarla al **```PC```**. La razón es todas las interrupciones o excepciones dependen de la etapa del pipeline en que se resuelvan y de su propio funcionamiento interno. Por razones de retrocompatibilidad con ARMv4 y 5, se asume un pipeline de 3 etapas, aun cuando los procesadores mas modernos tiene varias etapas mas. Por ello, antes utilizarlo para retornar se debe restar 4 u 8 a **```LR```**, o en otros casos como **```SVC```**, dejarlo tal cual. Esto se conoce como "fuga de abstracción" del pipeline en ARMv7. En buen romance.... **¡Horrible!**: Enchastrar la arquitectura con dependencias de la organización rompe todos los libros conocidos. Lo paga el programador teniendo que involucrarse en cuestiones que no debería.
